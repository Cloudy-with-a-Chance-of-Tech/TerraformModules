...
| <a name="input_ddos_protection_plan"></a> [ddos\_protection\_plan](#input\_ddos\_protection\_plan) | DDoS protection plan configuration. | <pre>object({<br>    id     = string<br>    enable = bool<br>  })</pre> | `null` | no |
| <a name="input_dns_servers"></a> [dns\_servers](#input\_dns\_servers) | List of IP addresses of DNS servers for the virtual network. If empty, uses Azure's default DNS (168.63.129.16). | `list(string)` | <pre>[<br>  "168.63.129.16"<br>]</pre> | no |
...
| <a name="input_location"></a> [location](#input\_location) | The Azure region where the virtual network will be created. | `string` | `"southcentralus"` | no |
...
| <a name="input_tags"></a> [tags](#input\_tags) | A map of tags to assign to the virtual network. | `map(string)` | <pre>{<br>  "ManagedBy": "Terraform"<br>}</pre> | no |
| <a name="input_virtual_network_name"></a> [virtual\_network\_name](#input\_virtual\_network\_name) | The name of the virtual network to create. If the name doesn't start with 'vnet-', it will be automatically prepended. | `string` | `"vnet-default-name"` | no |
